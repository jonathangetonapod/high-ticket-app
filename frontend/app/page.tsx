...
function getStatusBadge(status: string | number): { color: string; label: string; icon: string } {
  // Convert numeric status codes to string labels (Instantly API uses numbers)
                ...
                <div className="grid gap-6">
                  ...
                  {formData.clientId && campaigns.length > 0 && (
                    <>
                      ...
                      <Card className="bg-gradient-to-br from-blue-50 to-indigo-50 border-blue-300 border-2">
                        <CardContent className="p-5">
                          <div className="flex items-start gap-3 mb-3">
                            ...
                            <div className="flex-1">
                              <p className="font-bold text-gray-900 mb-1">First, load all campaigns</p>
                              <p className="text-sm text-gray-600">
                                Click below to fetch email sequences from all {campaigns.length} campaigns
                              </p>
                            </div>
                          </div>
                          ...
                          <Button
                            ...
                            className="w-full shadow-lg bg-blue-600 hover:bg-blue-700 text-white font-semibold"
                            size="lg"
                          >
                            {loadingCampaignDetails ? (
                              ...
                              <>
                                <Mail size={18} />
                                Load All Campaigns for {formData.clientName}
                              </>
                            )}
                          </Button>
                              ...
                              <div className="flex items-center gap-2 p-3 bg-green-50 border border-green-200 rounded-lg">
                                ...
                                <div className="flex-1">
                                  <p className="text-sm font-bold text-green-900">
                                    ✓ Successfully loaded {campaignDetails.length} campaigns
                                  </p>
                                  <p className="text-xs text-green-700 mt-0.5">
                                    {campaignDetails.reduce((sum, c) => sum + c.sequences.length, 0)} email sequences ready to review
                                  </p>
                                </div>
                              </div>
                            </div>
                      ...
                      {campaignDetails && campaignDetails.length > 0 && (
                        <>
                          <Card className={`border-2 transition-all ${selectedCampaigns.size > 0 ? 'border-green-500 bg-green-50/40' : 'border-orange-400 bg-orange-50/40'}`}>
                            <CardContent className="p-5 space-y-4">
                              {/* Step 2 Header */}
                              <div className="flex items-start gap-3">
                                <div className="w-8 h-8 rounded-full bg-orange-600 text-white flex items-center justify-center flex-shrink-0 font-bold text-sm">
                                  2
                                </div>
                                <div className="flex-1">
                                  <p className="font-bold text-gray-900 mb-1">Select campaigns to analyze</p>
                                  <p className="text-sm text-gray-600">
                                    Choose which campaigns you want to validate below
                                  </p>
                                </div>
                              </div>
                              ...
                              <div className="flex items-center justify-between p-3 bg-white rounded-lg border-2 border-gray-200">
                                <div className="flex items-center gap-3">
                                  <input
                                    ...
                                    checked={selectedCampaigns.size === campaignDetails.length}
                                    onChange={(e) => {
                                      if (e.target.checked) {
                                        setSelectedCampaigns(new Set(campaignDetails.map(c => c.campaign_id)))
                                      } else {
                                  ...
                                  />
                                  <label htmlFor="select-all-campaigns" className="text-sm font-bold text-gray-900 cursor-pointer">
                                    Select All
                                  </label>
                                </div>
                                <div className={`flex items-center gap-2 px-3 py-1.5 rounded-lg font-bold ${
                                  ...
                                  {selectedCampaigns.size > 0 && <CheckCircle size={16} />}
                                  <span className="text-sm">
                                    {selectedCampaigns.size} of {campaignDetails.length} selected
                                  </span>
                                </div>
                              </div>
                              ...
                              {selectedCampaigns.size > 0 ? (
                                <div className="p-4 bg-green-50 border-2 border-green-200 rounded-lg">
                                  ...
                                  <div className="flex flex-wrap gap-2">
                                    {Array.from(selectedCampaigns).slice(0, 4).map(campaignId => {
                                      const campaign = campaignDetails.find(c => c.campaign_id === campaignId)
                                      return campaign ? (
                                        <span key={campaignId} className="text-xs px-2.5 py-1 bg-white text-gray-800 rounded-md border border-green-300 font-medium">
                                          {campaign.campaign_name}
                                        </span>
                                      ) : null
                                    })}
                                    {selectedCampaigns.size > 4 && (
                                      <span className="text-xs px-2.5 py-1 bg-green-100 text-green-700 rounded-md border border-green-300 font-bold">
                                        +{selectedCampaigns.size - 4} more
                                      </span>
                                    )}
                                  </div>
                                </div>
                                ...
                                <div className="p-4 bg-orange-50 border-2 border-orange-200 rounded-lg">
                                  ...
                                  <p className="text-xs text-orange-700">
                                    ⬇️ Check the boxes below to select which campaigns you want to validate
                                  </p>
                                </div>
                              )}
                            </CardContent>
                      ...
                      {/* Step 3: Campaign List Header with Expand/Collapse All */}
                      {campaignDetails && campaignDetails.length > 0 && (
                              ...
                              <div className="flex items-center gap-3">
                                ...
                                <div>
                                  <p className="text-sm font-bold text-gray-900">
                                    Review and select campaigns
                                  </p>
                                  <p className="text-xs text-gray-600 mt-0.5">
                                    {campaignDetails.length} campaigns loaded • {expandedCampaigns.size} expanded
                                  </p>
                                </div>
                              </div>
                              <Button
                      ...
                      {campaignDetails && campaignDetails.map((campaign, campaignIndex) => {
                        ...
                        const isSelected = selectedCampaigns.has(campaign.campaign_id)
                        ...
                        return (
                          <Card
                            ...
                            className={`shadow-md transition-all hover:shadow-lg ${
                              isSelected
                                ? 'border-green-500 border-3 ring-4 ring-green-100 bg-green-50/30'
                                : 'border-gray-300 border-2 bg-white'
                            }`}
                          >
                            <CardContent className="p-0">
                              {/* Campaign Header */}
                              <div className={`w-full p-4 flex items-center justify-between transition-colors ${
                                ...
                                <div className="flex items-center gap-4 flex-1">
                                  ...
                                  <div className="flex-shrink-0">
                                    <input
                                      ...
                                      onChange={(e) => {
                                        ...
                                        if (e.target.checked) {
                                          newSelected.add(campaign.campaign_id)
                                        } else {
                                          newSelected.delete(campaign.campaign_id)
                                        }
                                        setSelectedCampaigns(newSelected)
                                      }}
                                      onClick={(e) => e.stopPropagation()}
                                      className={`w-6 h-6 cursor-pointer rounded border-2 ${
                                        isSelected
                                          ? 'text-green-600 border-green-600'
                                          : 'text-blue-600 border-gray-400'
                                  ...
                                  <div className="flex-1 min-w-0">
                                    ...
                                    <div className="flex items-center gap-3 mt-1">
                                      <p className="text-xs text-gray-600 font-medium">
                                        📧 {campaign.sequences.length} email{campaign.sequences.length > 1 ? 's' : ''}
                                      </p>
                                      <span className={`text-xs px-2 py-0.5 rounded-md border font-medium ${statusBadge.color}`}>
                                  ...
                                  <button
                                    ...
                                    className="flex items-center gap-2 px-3 py-2 rounded-lg border-2 border-gray-300 bg-white hover:bg-gray-50 transition-colors flex-shrink-0"
                                  >
                                          ...
                                          <div className="space-y-3">
                                            ...
                                            <div className="flex items-center justify-between flex-wrap gap-2">
                                              <div className="flex items-center gap-2">
                                                ...
                                                {/* Bison-specific: Thread Reply indicator */}
                                                {seq.thread_reply && (
                                                  <span className="text-xs px-2 py-1 rounded-md bg-blue-100 text-blue-700 border border-blue-200 font-medium">
                                                    📧 Thread Reply
                                                  </span>
                                                )}
                                                {/* Bison-specific: Variant indicator */}
                                                {seq.variant && (
                                                  <span className="text-xs px-2 py-1 rounded-md bg-amber-100 text-amber-700 border border-amber-200 font-medium">
                                                    🔀 Variant
                                                  </span>
                                                )}
                                              </div>
                                            ...
                                            <div>
                                              <div className="flex items-center justify-between mb-1.5">
                                                ...
                                                {spintaxInfo.hasSpintax ? (
                                                  <span className="text-xs px-2 py-0.5 rounded-md bg-green-50 text-green-700 border border-green-300 font-medium flex items-center gap-1">
                                                    <CheckCircle size={12} />
                                                    Contains spintax
                                                  </span>
                                                ) : (
                                                  <span className="text-xs px-2 py-0.5 rounded-md bg-red-50 text-red-700 border border-red-300 font-medium flex items-center gap-1">
                                                    <AlertTriangle size={12} />
                                                    Missing spintax
                                                  </span>
                                                )}
                                              </div>
                                              <div className={`text-sm text-gray-700 leading-relaxed p-3 rounded border max-h-64 overflow-y-auto ${
                                                spintaxInfo.hasSpintax ? 'bg-gray-50 border-gray-200' : 'bg-red-50/30 border-red-200'
                                              }`}>
                                                {highlightMergeFields(cleanBody)}
                                              </div>
                                            </div>

                                            {/* Spintax Word Options Display */}
                                            {spintaxInfo.hasSpintax ? (
                                              <div className="pt-2 border-t border-gray-200">
                                                <p className="text-xs text-gray-500 mb-2 font-medium flex items-center gap-1">
                                                  <span>Spintax word options:</span>
                                                  <span className="text-gray-400 font-normal">(randomized text alternatives)</span>
                                                </p>
                                              ...
                                              <div className="pt-2 border-t border-red-200 bg-red-50/50 p-3 rounded">
                                                <div className="flex items-start gap-2">
                                                  ...
                                                  <div className="flex-1">
                                                    ...
                                                    <p className="text-xs text-red-700 leading-relaxed">
                                                      Email body must contain spintax for deliverability. Add text variations using {'{'}option1|option2{'}'} syntax to randomize content and avoid spam filters.
                                                    </p>
                                                  </div>
                                                </div>
                                              </div>
                                            ...
                                            {seq.variants && seq.variants.length > 1 && (
                                              <div className="pt-3 border-t-2 border-purple-200">
                                                <div className="flex items-center gap-2 mb-3">
                                                  <span className="text-xs font-bold px-2.5 py-1 rounded-md bg-purple-100 text-purple-700 border border-purple-200">
                                                    {seq.variants.length} A/B Test Variants
                                                  </span>
                                                  <p className="text-xs text-gray-500">
                                                    Showing variant 1 above • {seq.variants.length - 1} more below
                                                  </p>
                                                </div>
                                                <div className="space-y-3">
                                                  {seq.variants.slice(1).map((variant: any, vIdx: number) => {
                                                    const variantBody = htmlToText(variant.body || '')
                                                    return (
                                                      <div key={vIdx} className="bg-purple-50/50 border border-purple-200 rounded-lg p-3 space-y-2">
                                                        ...
                                                        {variant.subject && (
                                                          <div>
                                                            <p className="text-xs text-purple-700 mb-1 font-medium">Subject:</p>
                                                            <div className="text-xs font-semibold text-gray-900 bg-white p-2 rounded border border-purple-200">
                                                              {highlightMergeFields(variant.subject)}
                                                            </div>
                                                          </div>
                                                        )}
                                                        <div>
                                                          <p className="text-xs text-purple-700 mb-1 font-medium">Body:</p>
                                                          <div className="text-xs text-gray-700 leading-relaxed bg-white p-2 rounded border border-purple-200 max-h-48 overflow-y-auto">
                                                            {highlightMergeFields(variantBody)}
                                                          </div>
                                                        </div>
                                                      </div>
                                                    )
